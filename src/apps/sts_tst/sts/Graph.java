...
public class Graph extends DesignPanel {
    ...
    protected void config() {
        ...
        addDATAin("New k(not required):", h2);
        integerTextField(getDataInput().get(11));
        addDATAin("Start k:", h2);
        ...
        switch (getSelectedOption()) {
            case 0 -> {
                try {
                    ...
                    for (int i = 0; i < getHorizontalValues().size(); i++) {
                        verticalValues.add(SquareMatrix.calculateComplexFp(getHorizontalValues().get(i), aux));
                    }
                } catch (NumberFormatException ignore) {
                    for (int i = 0; i < getHorizontalValues().size(); i++) {
                        verticalValues.add(SquareMatrix.calculateComplexFp(getHorizontalValues().get(i)));
                    }
                }
                JOptionPane.showMessageDialog(this,
                        setTextSize("Warning k &#60 " + SquareMatrix.getKInterval()[0] + " values are included", h2));
            }
            case 1 -> {
                try {
                    double aux = Double.parseDouble(getDataInput().get(7).getText());
                    for (int i = 0; i < getHorizontalValues().size(); i++) {
                        verticalValues.add(SquareMatrix.calculateComplexFB(getHorizontalValues().get(i), aux));
                    }
                } catch (NumberFormatException ignore) {
                    for (int i = 0; i < getHorizontalValues().size(); i++) {
                        verticalValues.add(SquareMatrix.calculateComplexFB(getHorizontalValues().get(i)));
                    }
                }
                JOptionPane.showMessageDialog(this,
                        setTextSize("Warning k &#60 " + SquareMatrix.getKInterval()[0] + " values are included", h2));
            ...
            case 2 -> {
                try {
                    ...
                    for (int i = 0; i < getHorizontalValues().size(); i++) {
                        verticalValues.add(SquareMatrix.calculateBFp(getHorizontalValues().get(i), (int) aux));
                    }
                } catch (NumberFormatException ignore) {
                    for (int i = 0; i < getHorizontalValues().size(); i++) {
                        verticalValues.add(SquareMatrix.calculateBFp(getHorizontalValues().get(i)));
                    }
                }
                if (SquareMatrix.Output.k < SquareMatrix.getKInterval()[0] && getDataInput().get(11).getText().isEmpty()) {
                    JOptionPane.showMessageDialog(this,
                            setTextSize("Warning k &#60 " + SquareMatrix.getKInterval()[0] + " values are included", h2));
                } else if (!getDataInput().get(11).getText().isEmpty() &&
                        Integer.parseInt(getDataInput().get(11).getText()) < SquareMatrix.getKInterval()[0]) {
                    JOptionPane.showMessageDialog(this,
                            setTextSize("Warning k &#60 " + SquareMatrix.getKInterval()[0] + " values are included", h2));
                }
            }
            case 3 -> {
                try {
                    ...
                    for (int i = 0; i < getHorizontalValues().size(); i++) {
                        verticalValues.add(SquareMatrix.calculateBFk(getHorizontalValues().get(i).intValue(), aux));
                    }
                } catch (NumberFormatException ignore) {
                    for (int i = 0; i < getHorizontalValues().size(); i++) {
                        verticalValues.add(SquareMatrix.calculateBFk(getHorizontalValues().get(i).intValue()));
                    }
                }
                if (Integer.parseInt(getDataInput().get(12).getText()) < SquareMatrix.getKInterval()[0]) {
                    JOptionPane.showMessageDialog(this,
                            setTextSize("Warning k &#60 " + SquareMatrix.getKInterval()[0] + " values are included", h2));
                }
            }
    ...
    private int getSelectedOption() {
        return STS.getOutput().getSelectedOption();
    }
