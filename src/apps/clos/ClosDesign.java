...
public class ClosDesign extends JPanel {
    ...
    public void setOutputData(ArrayList<String> outputData, ArrayList<JTextField> inputData) {
        for(int i=0; i<outputData.size(); i++) {
            output.getOutputData().get(i).setText(outputData.get(i));
            if(i == 1 && Double.parseDouble(outputData.get(i)) < Double.parseDouble(outputData.get(3))/2.0) {
                output.getOutputData().get(i).setForeground(Color.red);
            }
            else if(i == 1 && Double.parseDouble(outputData.get(i)) >= Double.parseDouble(outputData.get(3))/2.0) {
                output.getOutputData().get(i).setForeground(Color.black);
            }
        }
...
class Input extends JPanel {
    ...
    private void setup() {
        ...
        calculate.addActionListener(e -> {
            if(filterInputData()) {
                ((ClosDesign) this.getParent().getParent()).setOutputData(calculate(), inputData);
            }
        });
    }
    ...
    public Boolean filterInputData() {
        try {
            if(Integer.parseInt(inputData.get(0).getText()) < 2 || (Integer.parseInt(inputData.get(0).getText()) &
                    (Integer.parseInt(inputData.get(0).getText()) - 1)) != 0) {
                JOptionPane.showMessageDialog(this,
                        "<html><h2 style='color:red'>N must be higher than 0 and be a power of 2</h2></html>");
                return false;
            }
...
class Output extends JPanel {
    ...
    public void setInputData(ArrayList<JTextField> inputData) {
        this.inputData = (ArrayList)inputData.clone();
    }

    public ArrayList<JTextField> getInputData() {
        return inputData;
    }
}
...
class Graph extends JPanel {
    ...
    private void setup() {
        ...
        show.addActionListener(e -> {
            if(filterInputData()) {
                ArrayList<ArrayList<Double>> graphVars = initGraphVars();
                new GridGraph(graphVars.get(0), graphVars.get(1));
            }
        });
    }
    ...
    private Boolean filterInputData() {
        if(!((Input)((JComponent)this.getParent().getParent().getComponent(1)).
                getComponent(1)).filterInputData()) {
            return false;
        }
        else {
            ((Input)((JComponent)this.getParent().getParent().getComponent(1)).
                    getComponent(1)).getCalculate().doClick();
        }
        int type = 0;
        ...
        ArrayList<JTextField> outputData = ((Output)((JComponent)this.getParent().getParent().getComponent(1)).
                getComponent(3)).getOutputData();
        double k;
        try {
            k = Double.parseDouble(outputData.get(1).getText());
            for(int i=type*4; i<type*4+4; i++) {
                if(i == 14 || i == 13 || i == 12 || i == 11) {
                    if(i == 11) {
                        if(!inputField.get(i).getText().isEmpty()) {
                            k = Double.parseDouble(inputField.get(i).getText());
        ...
        double beta = Double.parseDouble(outputData.get(3).getText())/2.0;
        if(k < beta) {
            JOptionPane.showMessageDialog(this,
                    "<html><h2 style='color:red'>Warning k &#60 " + beta + " values are included</h2></html>");
        }
        return true;
    }

    private ArrayList<ArrayList<Double>> initGraphVars() {
        int type = 0;
        for(int i=0; i<this.type.size(); i++) {
            if(this.type.get(i).isSelected()) {
                type = i;
        ...
        BigDecimal start = new BigDecimal(inputField.get(4*type).getText());
        BigDecimal end = new BigDecimal(inputField.get(4*type+1).getText());
        BigDecimal step = new BigDecimal(inputField.get(4*type+2).getText());
        while(start.compareTo(end) < 0) {
            horizontalValues.add(start.doubleValue());
            start = start.add(step);
        }
        horizontalValues.add(end.doubleValue());
        ArrayList<JTextField> inputData = ((Output)((JComponent)this.getParent().getParent().getComponent(1)).
                getComponent(3)).getInputData();
        ArrayList<JTextField> outputData = ((Output)((JComponent)this.getParent().getParent().getComponent(1)).
                getComponent(3)).getOutputData();
        switch(type) {
            case 0:
                for(int i=0; i<horizontalValues.size(); i++) {
                    ...
                    double Bmax;
                    try {
                        Bmax = Double.parseDouble(inputField.get(3).getText());
                    } catch (NumberFormatException ignore) {
                        Bmax = Double.parseDouble(inputData.get(2).getText());
                    }
                    double n, calculatedB, k, Nx;
                    n = Double.parseDouble(outputData.get(3).getText());
                    k = 1;
                    while (true) {
                        calculatedB = Tools.pow(1 - Tools.pow(1 - (p * n) / k, 2), (int) k);
                        if (calculatedB <= Bmax && calculatedB >= 0) {
            ...
            case 1:
                for(int i=0; i<horizontalValues.size(); i++) {
                    ...
                    double p;
                    try {
                        p = Double.parseDouble(inputField.get(7).getText());
                    } catch(NumberFormatException ignore) {
                        p = Double.parseDouble(inputData.get(1).getText());
                    }
                    double Bmax = horizontalValues.get(i);
                    double n, calculatedB, k, Nx;
                    n = Double.parseDouble(outputData.get(3).getText());
                    k = 1;
                    while (true) {
            ...
            case 2:
                for(int i=0; i<horizontalValues.size(); i++) {
                    double p = horizontalValues.get(i);
                    double n = Double.parseDouble(outputData.get(3).getText());
                    double k;
                    try {
                        k = Double.parseDouble(inputField.get(11).getText());
                    } catch(NumberFormatException ignore) {
                        k = Double.parseDouble(outputData.get(1).getText());
                    }
            ...
            case 3:
                for(int i=0; i<horizontalValues.size(); i++) {
                    ...
                    double p;
                    try {
                        p = Double.parseDouble(inputField.get(15).getText());
                    } catch(NumberFormatException ignore) {
                        p = Double.parseDouble(inputData.get(1).getText());
                    }
                    double calculatedB = Tools.pow(1 - Tools.pow(1 - (p * n) / k, 2), (int) k);
