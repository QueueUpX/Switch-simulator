package apps.student.sts;

import apps.student.GridGraph;
import apps.student.STS_TST;
import apps.student.SwitchDesign;
import tools.Tools;
...
public class STS extends JPanel {
        ...
        next.addActionListener(e -> {
            ...
            if(this.getComponent(0) instanceof Graph) {
                if(filterGraphData(graphSettings.getDataInput(), output.getType())) {
                    BigDecimal start = new BigDecimal(graphSettings.getDataInput().get(0).getText());
                    BigDecimal end = new BigDecimal(graphSettings.getDataInput().get(1).getText());
                    BigDecimal step = new BigDecimal(graphSettings.getDataInput().get(2).getText());
    ...
    private Boolean filterGraphData(ArrayList<JTextField> inputs, int option) {
        ...
        double k = Double.parseDouble(output.getDataOutput().get(4).getText());
        switch(option) {
            case 0:
                try {
            ...
            case 2:
                try {
                    ...
                    else if(!inputs.get(3).getText().isEmpty()) {
                        k = Double.parseDouble(inputs.get(3).getText());
                    }
                    if(!inputs.get(4).getText().isEmpty() && Double.parseDouble(inputs.get(4).getText()) > 1) {
                        JOptionPane.showMessageDialog(this,
                                "<html><h2 style='color:red'>'New max B[0, 1]' field is invalid</h2></html>");
            ...
            case 3:
                try {
                    if(Integer.parseInt(inputs.get(0).getText()) == 0) {
                        ...
                        k = Double.parseDouble(inputs.get(0).getText());
                    }
        ...
        double beta = Double.parseDouble(output.getDataOutput().get(0).getText())/2.0;
        if(k < beta) {
            JOptionPane.showMessageDialog(this,
    ...
    private void SquareMatrix(ArrayList<JTextField> inputs, ArrayList<JTextField> outputs) {
        ...
        if(k < n/2) {
            outputs.get(4).setForeground(Color.red);
        }
        else {
            outputs.get(4).setForeground(Color.black);
        }
...
class Input extends JPanel {
    ...
    Input() {
        ...
        this.setPreferredSize(new Dimension(850, 625));
...
class Output extends JPanel {
    ...
    private void setup() {
        ...
        for(int i=0; i<4; i++) {
            type.add(new JRadioButton(DATA[6+i]));
            type.get(i).setFocusable(false);
            type.get(i).setMaximumSize(new Dimension(300, 35));
        }
    }
